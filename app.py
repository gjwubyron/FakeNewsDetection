...
app = Flask(__name__)
model = BertForSequenceClassification.from_pretrained('F:\\bert-vs\\bert-cache',local_files_only=True,
                                                      num_labels=2,
                                                      output_attentions=False,
                                                      output_hidden_states=False)
model.load_state_dict(torch.load('finetuned_BERT_epoch_4.model', map_location=torch.device('cpu')))

tokenizer = BertTokenizer.from_pretrained('F:\\bert-vs\\bert-cache', 
                                          do_lower_case=True)
...
def predict():
    ...
    print(key[0]+str(len(key[0].split())))
